...
export function LogEntryFormClient(props: {
  mode: { kind: "create"; backTo: string } | { kind: "edit"; logEntryId: string; redirectTo: string };
  ...
  const previewUri =
    hasExistingPhoto && !photoDeleted
      ? photoEdit.uri
      : (!hasExistingPhoto && canStageNewPhoto ? newPhoto.uri.trim() : "");

  const previewAlt =
    hasExistingPhoto && !photoDeleted
      ? (photoEdit.alt || photoEdit.caption || title || "Photo")
      : (!hasExistingPhoto && canStageNewPhoto ? (newPhoto.alt || newPhoto.caption || title || "Photo") : "");
    ...
    try {
      ...
      if (hasExistingPhoto) {
        ...
        if (photoDeleted) {
          ...
          if (photoEdit.caption !== (initial.photo!.caption ?? "")) patch.caption = photoEdit.caption;
    ...
    <Container size={900} py="xl">
      <Stack gap="lg">
        ...
        <Stack gap={6}>
          ...
          <Title order={2}>{props.mode.kind === "create" ? "Create log entry" : (initial.title ?? "Edit log")}</Title>
        </Stack>
        ...
        <Paper withBorder radius="lg" p="lg">
          <Stack gap="md">
            ...
            <TextInput
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHappenedAtLocal(e.target.value)}
            />
        ...
        <Paper withBorder radius="lg" p="lg">
          <Stack gap="md">
            ...
            {previewUri ? (
              <Card withBorder radius="lg" padding="sm" style={{ opacity: hasExistingPhoto && photoDeleted ? 0.55 : 1 }}>
                <Card.Section>
